...
"))
        ...
        public NetConnectionStatus TestNetConnection(string ComputerName, int? Port)
        {
            string script = @"
param ($ComputerName,
       [int] $Port)
...
$properties = @(
""ComputerName"",
@{Name=""RemoteAddress"";Expression={$_.RemoteAddress | ? { $_} | % { $_.ToString() } | select -First 1}},
""PingSucceeded"",
""TcpClientSocket"",
""TcpTestSucceeded"",
""RemotePort"",
...
@{Name=""SourceAddress"";Expression={$_.SourceAddress | ? { $_} | % { $_.ToString() } | select -First 1}},
""NameResolutionSucceeded"",
""NetworkIsolationContext""
)

$result = Test-NetConnection -WarningAction SilentlyContinue @params | select $properties
## This result binding actually works.
## TODO make this a generic type of SimpleScriptRunner that will offer a TypedResult T.
if ($result) {
    $realResult = [LaparoscopeShared.Models.NetConnectionStatus]::new()
    foreach ($p in $result.PSObject.Properties.Name) {
        $realResult.$p = $result.$p
    }
    return $realResult
}
                ";
            ...
            {
                runner.Parameters.Add("ComputerName", ComputerName);
                if (Port != null) { runner.Parameters.Add("Port", Port); }
                runner.Run();
                if (runner.HadErrors)
                {
                    var err = runner.LastError ?? new Exception("Encountered an error in PowerShell but could not capture the exception.");
                ...
                var resultValues = runner.Results.CapturePSResult<NetConnectionStatus>()
                    .Where(x => x is NetConnectionStatus)
                    .Cast<NetConnectionStatus>().FirstOrDefault();
